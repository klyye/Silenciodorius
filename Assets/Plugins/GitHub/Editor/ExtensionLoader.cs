...
namespace GitHub.Unity
{
    ...
    public class ExtensionLoader : ScriptableSingleton<ExtensionLoader>
    {
        ...
        {
            "System.Threading.dll", "AsyncBridge.Net35.dll", "ReadOnlyCollectionsInterfaces.dll",
            "GitHub.Api.dll", "GitHub.Unity.dll"
        };
        ...
        {
            get { return Environment.GetEnvironmentVariable(GITHUB_UNITY_DISABLE) == "1"; }
        }
        ...
        private static void Initialize()
        {
            ...
            //if (!ExtensionLoader.instance.Initialized)
            {
                var scriptPath = Path.Combine(Application.dataPath,
                    "Editor" + Path.DirectorySeparatorChar + "GitHub.Unity" +
                    Path.DirectorySeparatorChar + "EntryPoint.cs");
        ...
        private static void ToggleAssemblies(string path, string[] assemblies, bool enable)
        {
            foreach (var file in assemblies)
            {
                ...
                {
                    Debug.LogFormat(
                        "GitHub for Unity: Could not find importer for {0}. Some functionality may fail.",
                        filepath);
                    continue;
                }
